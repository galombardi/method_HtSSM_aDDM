...
paramSim<- c(rep(0,4),ndt*1000)
  ...
  for (s in 1:length(subs)) {
      ...
      # draw one parameter from each distribution
      param<-paramSim
      param[1]<-sample(dDist,1, replace = TRUE)
      param[2]<-sample(thetaDist,1, replace = TRUE)
      param[3]<-sample(noiseDist,1, replace = TRUE)
      param[4]<-sample(biasDist,1, replace = TRUE)
      ...
      DataFit$drift[DataFit$SubjectNum == subject]<- param[1]
      DataFit$theta[DataFit$SubjectNum == subject]<- param[2]
      DataFit$noise[DataFit$SubjectNum == subject]<- param[3]
      DataFit$bias[DataFit$SubjectNum == subject]<- param[4]
  }
  ...
  results <- run.jags(model=file.path(main_folder,"HaDDM_model_RiskySafe.txt"), monitor=monitor, data=dat, n.chains=3, inits=c(inits1,inits2, inits3), plots = TRUE, method="parallel", module="wiener", burnin=50000, sample=10000, thin=10)
    ...
    save(results,params,DataFit, file=file.path(main_folder,"results_HaDDM_heterogParams.RData"))
    write.csv(suuum, file=file.path(main_folder,"results_HaDDM_heterogParams.csv"))
