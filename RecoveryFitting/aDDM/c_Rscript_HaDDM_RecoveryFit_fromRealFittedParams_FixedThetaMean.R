...
for (s in 1:length(subs)) {
    ...
    DataFit$theta[DataFit$SubjectNum==subject] = mean(chain1[,"thetaGaze.mu"])
    ...
    theta <- mean(chain1[,"thetaGaze.mu"])
...
save(results,DataFit, file=file.path(main_folder,"results_HaDDM_fromRealFittedParams_thetaFixedMean.RData"))
write.csv(suuum, file=file.path(main_folder,"results_HaDDM_fromRealFittedParams_thetaFixedMean.csv"))
