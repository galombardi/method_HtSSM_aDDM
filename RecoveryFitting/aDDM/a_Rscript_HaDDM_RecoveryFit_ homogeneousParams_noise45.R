...
d <- (c(rep(6,4),rep(10,4),rep(14,4)))/1000
theta <- rep(c(0.2,0.4,0.6,0.8),3)
noise <- 0.45/sqrt(1000)
bias <- 0
ndt<- 0.3


for (p in (1:length(d))){
  paramSim<- c(d[p], theta[p],noise,bias,ndt*1000)
...
# run the model
  ...
  params<-c(d[p],theta[p],noise,bias,ndt)
  save(results,params,DataFit, file=file.path(main_folder,"results_HaDDM_homogParams_noise45_",toString(p), ".RData"))
  write.csv(suuum, file=file.path(main_folder,"results_HaDDM_homogParams_noise45_",toString(p),".csv"))
}
