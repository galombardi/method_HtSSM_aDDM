...
for (s in 1:length(subs)) {
    ...
    DataFit$theta[DataFit$SubjectNum==subject] = 0.4
...
save(results,DataFit, file=file.path(main_folder,"results_HaDDM_fromRealFittedParams_thetaFixed4.RData"))
write.csv(suuum, file=file.path(main_folder,"results_HaDDM_fromRealFittedParams_thetaFixed4.csv"))
