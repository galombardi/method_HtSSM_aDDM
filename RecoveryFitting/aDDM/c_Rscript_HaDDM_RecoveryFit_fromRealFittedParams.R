...
subs <- unique(DataFit$SubjectNum)
  ...
  for (s in 1:length(subs)) {
    ...
    DataFit$theta[DataFit$SubjectNum==subject] = mean(chain[,c( paste( c("thetaGaze[",toString(s),"]"), collapse = ""))])
    ...
    theta <- unique(DataFit$theta[DataFit$SubjectNum==subject])
...
save(results,DataFit, file=file.path(main_folder,"results_HaDDM_fromRealFittedParams.RData"))
write.csv(suuum, file=file.path(main_folder,"results_HaDDM_fromRealFittedParams.csv"))
